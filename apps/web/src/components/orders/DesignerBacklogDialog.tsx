...
function buildStatusMeta(t: TFunction<'orders'>): Array<{
  ...
  return [
    {
      key: 'unassigned',
      code: 'unassigned',
      label: t('dialogs.designerBacklog.statusUnassigned'),
      cls: 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300',
    },
    {
      key: 'assigned',
      code: 'assigned',
      label: t('dialogs.designerBacklog.statusAssigned'),
      cls: 'bg-zinc-200 text-zinc-700 dark:bg-zinc-700 dark:text-zinc-200',
    },
    {
      key: 'inProgress',
      code: 'in-progress',
      label: t('dialogs.designerBacklog.statusInProgress'),
      cls: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300',
    },
    {
      key: 'rework',
      code: 'rework',
      label: t('dialogs.designerBacklog.statusRework'),
...
function ageMeta(ageDays: number, t: TFunction<'orders'>): { cls: string; label: string } {
  if (ageDays < 0)
    return {
      cls: 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400',
      label: t('dialogs.designerBacklog.ageUnknown'),
    };
  if (ageDays === 0)
    return {
      cls: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300',
      label: t('dialogs.designerBacklog.ageToday'),
    };
  if (ageDays <= 2)
    return {
      cls: 'bg-lime-100 text-lime-700 dark:bg-lime-900/40 dark:text-lime-300',
      label: t('dialogs.designerBacklog.ageDays', { count: ageDays }),
    };
  if (ageDays <= 7)
    return {
      cls: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300',
...
function fmtDay(day: string, t: TFunction<'orders'>): string {
  if (day === '__nodate__') return t('dialogs.designerBacklog.noProductionDate');
  const [, m, d] = day.split('-');
  return d && m ? `${d}/${m}` : day;
}
...
export function DesignerBacklogDialog({ open, onClose, onDrillDay }: Props) {
  ...
  const renderDesigner = (d: DesignerBacklogRow) => {
    ...
    const oldest = ageMeta(d.oldestAgeDays, t);
    return (
      <div key={d.userId} className="rounded-lg border border-border bg-card">
        ...
        >
          ...
          {d.oldestDay && (
            <span className={cn('rounded-full px-2 py-0.5 text-[10px] font-medium', oldest.cls)}>
              {t('dialogs.designerBacklog.oldestChip', { label: oldest.label })}
            </span>
          )}
          <span className="rounded-full bg-primary/10 text-primary px-2 py-0.5 text-[11px] font-bold tabular-nums">
            {t('dialogs.designerBacklog.totalChip', { count: d.total })}
          </span>
        </button>
        ...
        {isOpen && (
          <div className="border-t border-border/60 divide-y divide-border/40">
            {d.days.map((day) => {
              const age = ageMeta(day.ageDays, t);
              return (
                <div
                  key={day.day}
                ...
                >
                  ...
                  <button
                    ...
                    title={t('dialogs.designerBacklog.viewAllTasksTitle')}
                  >
                    <span className={cn('rounded px-1.5 py-0.5 text-[10px] font-medium w-[72px] text-center', age.cls)}>
                      {age.label}
                    </span>
                    <span className="font-mono font-medium w-12">{fmtDay(day.day, t)}</span>
                    <span className="font-bold tabular-nums w-14">
                      {t('dialogs.designerBacklog.taskCount', { count: day.total })}
                    </span>
                  </button>
                    ...
                    {STATUS_META.map((s) =>
                      day.byStatus[s.key] > 0 ? (
                        <button
                          ...
                          title={t('dialogs.designerBacklog.filterStatusTitle', { label: s.label })}
                        >
      ...
      <DialogContent className="max-w-5xl max-h-[88vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('dialogs.designerBacklog.title')}
          </DialogTitle>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-sm text-muted-foreground">{t('dialogs.designerBacklog.empty')}</div>
        ) : (
          <>
            <div className="flex items-center justify-between gap-3 text-xs text-muted-foreground">
              <span>
                <Trans
                  i18nKey="dialogs.designerBacklog.totalTasks"
                  ns="orders"
                  values={{ total: data.total }}
                  components={{ strong: <strong className="text-foreground" /> }}
                />
                {data.oldestDay && (
                  <>{t('dialogs.designerBacklog.oldestSuffix', { day: fmtDay(data.oldestDay, t) })}</>
                )}
              </span>
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={toggleAll}>
                  {allExpanded ? (
                    <>
                      <ChevronsDownUp size={13} className="mr-1" /> {t('dialogs.designerBacklog.collapseAll')}
                    </>
                  ) : (
                    <>
                      <ChevronsUpDown size={13} className="mr-1" /> {t('dialogs.designerBacklog.expandAll')}
                    </>
                  )}
                </Button>
                <div className="relative w-44">
                  ...
                  <Input
                    placeholder={t('dialogs.designerBacklog.searchPlaceholder')}
                    value={search}
            ...
            <div className="mt-2 flex-1 overflow-y-auto pr-1">
              {designers.length === 0 ? (
                <p className="py-6 text-center text-xs text-muted-foreground">
                  {t('dialogs.designerBacklog.noMatch')}
                </p>
              ) : (
