...
export function OrdersStatsBar({ counts }: { counts: CustomerOrderCounts | null }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
      ...
      <StatsCard icon={<Clock size={16} style={{ color: '#a16207' }} />} label={t('stats.pending')} value={n(counts?.pending)} color="#a16207" />
      <StatsCard icon={<Factory size={16} style={{ color: '#4338ca' }} />} label={t('stats.inProduction')} value={n(counts?.inProduction)} color="#4338ca" />
      <StatsCard icon={<CheckCircle2 size={16} style={{ color: '#15803d' }} />} label={t('stats.completed')} value={n((counts?.fulfilled ?? 0) + (counts?.completed ?? 0))} color="#15803d" />
      <StatsCard icon={<PauseCircle size={16} style={{ color: '#c2410c' }} />} label={t('stats.held')} value={n(counts?.held)} color="#c2410c" />
    </div>
  );
}
