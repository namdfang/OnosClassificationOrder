...
## 15. Deploy prod bằng tay (08/09/2026)

Nhóm chốt: **không có cổng CI, không tự deploy** — ai làm phần nào tự kiểm rồi tự đẩy lên prod. Bản CI + tự deploy dựng ngày 08/09/2026 đã gỡ bỏ ngay trong ngày.

- Đẩy prod: SSH vào máy prod → `cd /var/www/onosfactory/current && ./deploy.sh`.
- `deploy.sh` nhận **cả tên nhánh lẫn một SHA đầy đủ**: `./deploy.sh <sha>` để ra đúng một commit thay vì lấy đầu nhánh lúc build xong. Lùi: `./deploy.sh --rollback`. Xem đang chạy gì: `./deploy.sh --status`.
- Máy dev tự kéo nhánh `dev` mỗi phút (SellerPortal.md §8.2), không dính gì tới prod.
