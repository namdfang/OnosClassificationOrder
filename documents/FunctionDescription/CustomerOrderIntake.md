...
> **Đồng bộ tăng dần đơn Luồng A vào staging (07/09/2026):** `CustomerOrderService.syncLegacyOrdersIncremental()` — cron 5 phút (`customer-orders-legacy-sync`) + gọi trước mọi lần admin đọc (`listOrdersAdmin`/`getCountsAdmin`/`getStatsAdmin`, throttle 60 s). Quét `orders.createdAt ≥ watermark − 15'` (`system_configs.customer_orders_sync_watermark`), gom theo (userSku, email chữ thường), cặp chưa có trong `customers` thì upsert chỗ giữ như `CustomerService.sync`, rồi `syncLegacyOrdersForCustomer(customer, since)` (đã thêm tham số `since`; email khớp không phân biệt hoa/thường). Có đơn mới → xoá cache admin. Lý do: trước đây staging chỉ được bồi khi CHÍNH khách mở portal hoặc backfill 1 lần lúc boot → hub `/hub/orders` lọc "hôm nay" trống dù xưởng nhận ~200 đơn/ngày.
