...
- **`markToolCheckDone(id, ctx)`** (`order.service.ts`, cạnh nhóm hold): action "Đã soát xong" — validate marker hold + `assertNotHeld` + chưa hủy, gỡ `productionErrorSource`, set `currentFulfillmentStage=Print` + `readyForFulfill=false`, giữ `toolResultNote='error'`; designer từng làm xong → `designerStatus='rework'`; chưa có designer → `await autoAssignAfterImport([id])` rồi đọc lại `assignee` để trả outcome thật (`auto-assigned`/`backlog`). Controller `POST /orders/:id/tool-check-done` (`order.controller.ts`, sau `unhold`).
