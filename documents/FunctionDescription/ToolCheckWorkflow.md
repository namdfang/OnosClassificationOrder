...
- **Thống kê lỗi Soát tool** (ô riêng dưới cùng) — **LỖI DO NGƯỜI SOÁT TOOL TẠO RA** = đơn **TỪNG bị đánh `toolResultNote` (Note kq Tool) ≠ ok (≠ rỗng)**, **KỂ CẢ đơn đã sửa về 'ok'** (khác trước chỉ đếm lỗi đang chờ / trạng thái hiện tại). Đếm **số ĐƠN riêng biệt** (dedup theo `orderId`). Lọc kỳ theo `inProductionAt`. 3 cột:
  ...
  - **Nút "Xem theo khách hàng"** (header ô thống kê) — mở **modal full-screen** (`Dialog` override `w-screen h-screen`): liệt kê MỌI khách hàng (sort số đơn lỗi nhiều→ít), mỗi khách 1 card gồm header (userSku + userEmail + tổng đơn + nút **Mở rộng/Ẩn bớt** riêng) + 2 cụm **Sản phẩm lỗi** (mockup+level+tên+số đơn) và **Loại lỗi** (badge+số đơn), đều đếm đơn riêng biệt & sort nhiều→ít. Mỗi cụm list **cuộn** khi dài (ẩn bớt = `max-h-48 overflow-y-auto`, mở rộng = xổ hết) — state `expandedCust: Set<key>`. Xem toàn bộ khách cùng lúc, khỏi bấm từng khách. Không áp cross-filter (hiện mọi khách trong kỳ + server filter).
