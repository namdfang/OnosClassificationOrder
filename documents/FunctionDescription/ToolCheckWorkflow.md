...
Thẻ **`TodayReportCard`** (`apps/web/src/components/common/TodayReportCard.tsx` — dùng chung Soát tool / Designer / Fulfillment) render **trên đầu tab Soát tool** (`ToolCheckTab.tsx`, trên filter bar). Cố định **hôm nay** (giờ VN), độc lập với bộ lọc kỳ 7/14/30. 5 ô bấm được → bấm ô nào hiện list đơn dưới:

| Ô | Định nghĩa | Timestamp |
|---|---|---|
| Đã nhận | đơn vào SX hôm nay | `inProductionAt` |
| Làm được | đơn soát xong hôm nay | `toolCheckedAt` |
| Đã sửa lại | đơn lỗi tool-check → `'ok'` lại hôm nay | **OrderLog** (field=`toolResultNote`, after=`ok`, before≠rỗng/ok, createdAt∈today) |
| Tìm được lỗi | đơn dính lỗi tool-check hôm nay | `productionErrorSource='tool-check'` + `toolResultNote='error'` + `productionFirstErrorAt∈today` |
| Còn tồn | hôm nay (vào SX hôm nay chưa soát) / tổng (`toolResultNote` rỗng, đã vào SX) | `toolResultNote` snapshot + `inProductionAt` |

- **BE:** `GET /v1/designer/tool-check-today-report` → `DesignerStatsService.getToolCheckTodayReport()` (dùng `orderLogModel` cho "đã sửa lại"). `@Auth(TOOL_CHECK_ROLES)`.
- **FE service:** `RepositoryRemote.designer.toolCheckTodayReport()`.
