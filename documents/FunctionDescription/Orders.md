...
### 16.1 Overview
Mỗi hàng đơn (ở MỌI bảng order + card list) có nút **"..."** (`MoreHorizontal`) sticky cuối hàng — **CHỈ Admin** (`OrderRowActionsMenu` tự `usePermission().isAdmin` → non-admin render null). 2 action, mỗi cái mở dialog:
...
### 16.4 Gắn vào bảng
`OrderRowActionsMenu` + badge "Đã hủy" gắn ở: `OrderTableWorkshop`, `OrderFactoryTab` (Tab C), `ErrorLogTab`, `OrdersMiniTable` (Tab B), `PrintOrderTable` (qua `PrintWorkshopView.renderRowAction`), `ListOrderTab` (card — nút góc). Cột action **sticky right**; `onChanged` → refetch bảng tương ứng.
