...
- **Đơn đã hủy (`cancelledAt`) KHÔNG tính vào BẤT KỲ facet count nào** (toolResultNote/lỗi/assignee…): hằng `excludeCancelled = dto.cancelled===true ? {} : { cancelledAt: {$exists:false} }` spread vào MỌI match facet (+ `toolResultNoteNoneCount` + `unassignedSplit` + `heldCount`). Áp cho **cả 3 bảng** dùng `workshop-filters` (Workshop/ListOrder/Print); `getErrorLog` vốn đã tự loại cancelled. **Toggle "Đã hủy (N)"** (`data.cancelledCount`, nút `Ban` cạnh "Đang giữ", URL `wcancel`) → set `cancelled=true` để xem RIÊNG chỉ đơn hủy (facet lúc đó phản ánh phân bố đơn hủy) + chip "Trạng thái: Đã hủy". `buildOrderListFilter`: `dto.cancelled===true` → `cancelledAt:{$exists:true}` (list chỉ đơn hủy); không truyền → list vẫn hiện đơn hủy tô xám như cũ. DTO `GetProductionOrdersZod.cancelled` (`z.coerce.boolean`).
