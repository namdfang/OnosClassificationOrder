...
4. `POST /design-review/result` — sau khi client xử lý xong, gọi lại để lưu **Kết quả Tool** (`toolResult`) — tương đương thao tác tay ở cột "Kết quả Tool" (Danh sách đơn, `/orders?tab=list`). Đơn tự rời khỏi hàng đợi `next` ngay khi `toolResult` được set (không rỗng nữa). Optional `toolResultNote` ("Note kq Tool 1") — nếu truyền, ghi luôn field này và chạy CÙNG side-effect hook với sửa tay (xem §18.7). Optional `errorFile` ("File lỗi", chọn được nhiều lỗi cùng lúc) + `errorFileNote` ("Ghi chú", free text) — cùng cơ chế optional, dùng khi `toolResultNote='error'` (xem §18.7). Optional `printFileUrl` — Drive URL của **"File in"** (design đã duyệt), free text, cùng cơ chế optional, KHÔNG qua `workshop_config`/`updateField()` (xem §18.7).
...
| `printFileUrl` | `string \| null` | Không | Drive URL của **"File in"** (design đã duyệt, dùng ở stage "In" của Fulfillment) — free text, **KHÔNG** qua `workshop_config`, **KHÔNG** phải `OrderWorkshopField` nên **KHÔNG** đi qua `updateField()` (set thẳng bằng `findOneAndUpdate`, cùng cách `cuttingFileUrl` được set ở `applyCuttingFiles()`, §17-ish). Cùng cơ chế optional như `errorFileNote`. **Không truyền field này** → giữ nguyên hành vi cũ, KHÔNG đụng field. **Truyền** (kể cả `null`/`''` để xoá) → ghi đè. KHÔNG có side-effect hook, KHÔNG chạy `assertCanEditField`/`assertValueAllowed`/`assertNotHeld` (public API, field không nằm trong permission gate của Danh sách đơn). |

`productionId` không khớp đơn nào → 404. `toolResult` không khớp code `workshop_config` category `tool_result` đang active → 400. `toolResultNote`/`errorFile` (nếu có truyền) không khớp code active → 400. `errorFileNote`/`printFileUrl` không validate (free text, luôn 200 nếu qua được các field trước).
...
Request mẫu (Note kq Tool 1 = ok, kèm `printFileUrl` — lưu Drive URL file in):
```json
POST /v1/orders/design-review/result
{
  "productionId": "RR-03884-24456",
  "toolResult": "has-tool",
  "toolResultNote": "ok",
  "printFileUrl": "https://drive.google.com/file/d/xxxxx/view"
}
```
