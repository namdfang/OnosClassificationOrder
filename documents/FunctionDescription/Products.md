...
- **Transform** — `attribute_specifics`→`variations` (đủ 7 loại giá + ship EXPRESS_US/SBTT cost + package; attributes label fallback `name || type` vì data thật `name` rỗng); `print_areas`→`printArea` object giàu (§2.4a — key ngoài `PRODUCT_PRINT_AREA_KEYS` bị bỏ); `shortName` derive từ sku/identity (bỏ dấu + uppercase); `collection`/`category` name → find-or-create Collection/ProductCategory (đụng unique `shortName` với entry KHÁC TÊN → retry suffix `-XXXX` random, giữ entry riêng); `visible:false`→`status:inactive`.
