...
| **Khu bộ lọc** (PRD-1) | Panel ngay trên bảng, 6 ô nhìn thấy sẵn (grid 1→6 cột theo bề rộng), tổ hợp **AND**, lọc trên TOÀN BỘ dữ liệu qua API chứ không lọc trang hiện tại: **Tên sản phẩm** (`fullName`) · **Tên viết tắt** (`shortName`) · **Xưởng** (`factoryId`) · **Phòng** (`machineTypeId`) · **Loại vải** (`fabricType`) · **Trạng thái** (`status`). Hai ô chữ khớp *chứa chuỗi, không phân biệt hoa thường*, debounce 400ms (`useDebounce`) và là **hai điều kiện riêng** — gõ cả hai thì phải thỏa cả hai (khác `search` cũ vốn gộp `fullName`/`shortName`/`sku` bằng một `$or`; tham số `search` GIỮ NGUYÊN cho nơi khác gọi). Loại vải lấy option từ `workshopConfigStore` (category `fabric_type`, cùng nguồn với select inline trong bảng) cộng mục **"Chưa đặt loại vải"** = `fabricType=none` (`PRODUCT_FABRIC_TYPE_NONE`) cho sản phẩm bỏ trống trường này. Có bất kỳ ô nào đang đặt → hiện nút **Xóa bộ lọc**. Bộ lọc KHÔNG lưu vào URL, không nhớ giữa các lần vào trang. |
