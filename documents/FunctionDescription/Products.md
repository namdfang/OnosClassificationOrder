...
| Chức năng | Mô tả |
|-----------|-------|
| **Quyền theo vai** (AUTH-6) | **SuperAdmin / Admin / Manager**: xem + sửa đầy đủ như trước. **Support**: XEM được cả 4 tab, **không sửa được gì** — khu 7 nút công cụ bị ẩn, 6 select sửa tại chỗ bị `disabled`, nút ✏️/🗑️ mỗi dòng bị ẩn, nút Thêm/Sửa ở 3 tab Danh mục/Collection/Xưởng cũng ẩn; bộ lọc + phân trang vẫn dùng bình thường. Chặn hai lớp: giao diện qua hook `useProductWriteAccess`, API qua `@Auth` (route ghi vẫn chỉ Admin+Manager) — xem `Auth.md` §7.2. **Vai không có `page.products`** (Designer/DesignerLeader/Fulfillment...) và **Customer** vẫn bị API từ chối như trước. |
