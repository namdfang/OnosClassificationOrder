...
- **Drill-down (bấm con số → modal đơn):** MỌI con số > 0 đều click được → mở `<OrderListDialog>` (`apps/web/src/components/orders/OrderListDialog.tsx`) — modal full màn hình, bảng đơn layout **giống `OrderFactoryTab`** (cột Xưởng + nhóm cột nghiệp vụ `buildColGroups`/`GroupCellContent`, preview ảnh + lịch sử, phân trang 20/trang). Query build client-side qua `getOrders` (param `createdFrom`/`createdTo` = lọc `inProductionAt`; `sort=grouped`; + `type`/`customer` từ bộ lọc chung), map metric → `toolResultNote`:
