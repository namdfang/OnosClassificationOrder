...
Sau khi **soát tool xong** cho một đơn với `toolResultNote === 'error'` (hằng
`TOOL_CHECK_ERROR_CODE` trong `order.service.ts` — tức ĐÚNG lỗi thiết kế cần
designer sửa), hệ thống **tự động gán** đơn cho designer theo **chuỗi ưu tiên 3
mức** Admin cấu hình ở `/adm/settings/designer-assign`, **không cần gán tay**:
...
**CHỈ mã `'error'` mới auto-gán (TỰ ĐỘNG, ngay khi lưu kết quả tool)** — các mã
`toolResultNote` khác != `'ok'` (vd `'no-tool'` "Không có tool", `'no-pdf'`
"Không có file PDF", hoặc mã tuỳ biến Admin thêm qua `workshop_config` category
`tool_result_note`) **KHÔNG** tự gán designer ngay lúc lưu — đây không phải lỗi
thiết kế, đơn chỉ đang thiếu điều kiện soát (thiếu tool/PDF), auto-gán ngay sẽ
giao nhầm việc không thuộc trách nhiệm designer. Đơn với các mã này vẫn được
lưu bình thường, chỉ KHÔNG kích hoạt `autoAssignAfterImport()` từ hook. (Khác
nút **"Tự động gán"** thủ công ở `DesignerAssignBacklog.tsx` §3 — nút đó Admin/Leader
CHỦ ĐỘNG bấm để quét lại backlog, dùng filter rộng hơn `∉ [null,'','ok']`, xem §5.2.)
