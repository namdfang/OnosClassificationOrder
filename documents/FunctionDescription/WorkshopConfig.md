...
Module có **10 category** (9 category nghiệp vụ xưởng + 1 category `print_method` phục vụ catalog sản phẩm — xem `Products.md §2.4`), mỗi category có một trong 2 mode hiển thị. **Lưu ý:** Danh mục sản phẩm (product category) KHÔNG dùng module này — đó là entity riêng `ProductCategoryEntity`, xem `Products.md §4`.
...
| `production_error` | color badge | Lý do xưởng báo lỗi đơn hàng. **Required `errorSource`** = `designer | factory | tool-check` để dashboard stats phân loại + định tuyến rework. `tool-check` → đẩy về Support (xem `ToolCheckWorkflow.md`). |
| `print_method` | icon | Phương pháp in cho catalog khách hàng (DTG, DTF, Sublimation, Thêu...) — field `ProductConfig.printMethod`, hiển thị ở Customer Portal Catalog (`CustomerPortal.md §7`). |
...
```ts
{
  ...
  order: number;                     // sort
  isActive: boolean;
  errorSource?: 'designer' | 'factory' | 'tool-check';  // CHỈ category=production_error. Required khi tạo/sửa row production_error. 'designer' trigger rework auto cho task designer; 'factory' chỉ ghi stats; 'tool-check' đẩy về Support (soát tool).
  stage?: FulfillmentStage;          // CHỈ production_error — công đoạn SỞ HỮU lỗi (Stage Error Catalog, rỗng = lỗi chung). Xem StageErrorCatalog.md.
  reworkTarget?: 'tool-check' | 'designer' | FulfillmentStage; // CHỈ production_error + có stage — đích đẩy về khi quét QR lỗi. errorSource được BE TỰ SUY từ đây.
}
...
| GET | `/v1/workshop-config/stage-errors?stage=` | mọi user đã login | Stage Error Catalog: list lỗi 1 công đoạn (gồm cả inactive). Xem `StageErrorCatalog.md`. |
| POST | `/v1/workshop-config/stage-errors` | Admin/SuperAdmin/Manager/**Fulfillment** | Tạo lỗi công đoạn — Fulfillment bị ép `stage` = của mình, code tự sinh `se-<stage>-<n>`, errorSource suy từ `reworkTarget`. |
| PATCH | `/v1/workshop-config/stage-errors/:id` | như POST | CHỈ nhận `{ isActive }` (ẩn/hiện — đã thêm là không sửa tên/đích) — Fulfillment chỉ thao tác lỗi công đoạn mình. |
...
- **production_error:** 11 lý do với `errorSource` flag:
  ...
  - **`tool-check` source**: `tool-missing-file` (Thiếu file để in) → đẩy về Support
- **print_method:** 4 mã mặc định — `dtg` (DTG), `dtf` (DTF), `sublimation` (Sublimation/ép nhiệt), `embroidery` (Thêu)
...
- Tabs (10 tab tương ứng 10 category).
- Mỗi tab render component `CategoryEditor` với prop `category` + `mode: 'color' | 'icon'`.

### 5.2 `CategoryEditor.tsx`
- Bảng các mục: cột Hiển thị (badge màu hoặc icon), Tên (+ **badge `DES`/`XƯỞNG`/`SOÁT TOOL`/`? CHƯA GÁN`** cạnh tên khi category=production_error), Mã, Trạng thái, Action.
- Nút **Thêm** → mở Dialog form: Tên + Mã (auto-slug từ tên) + ColorPicker / IconPicker + **3 button toggle "Do designer"/"Do xưởng"/"Do soát tool"** (chỉ hiện cho category=production_error, required) + switch isActive.
