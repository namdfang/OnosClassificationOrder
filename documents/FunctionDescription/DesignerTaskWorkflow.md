...
>  - `GET /v1/designer/performance` + `/timeline/:userId` + `/team-daily-breakdown` + `/breakdown-filters` + `/daily-overview` + `/assign-backlog`
...
| GET | `/v1/designer/daily-overview?days=7\|14\|30` **hoặc `?from&to`** **+ `?type&customer`** | Admin/Manager/Leader/Designer | **Bảng tổng quan N ngày** (`inProductionAt`, tz VN) — 4 hàng: `total` (mọi đơn) / `unreviewed` (toolResultNote rỗng) / `error` (note ≠ ok, kèm `errorByNote[]`) / `backlog` (designerStatus ≠ done). Kèm `backlogByDesigner[]` (per-designer × assigned/inProgress/rework/rejected) + `unassignedBacklog` cho bảng con expand hàng Tồn. `type`/`customer` = filter chung. Cho tab Dashboard §0b. Xem `Dashboard.md` Tab D. |
