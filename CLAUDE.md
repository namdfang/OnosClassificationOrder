...
| **Vòng đời đơn** (Lifecycle dashboard — **CHỈ Admin**; phễu rắn bò 9 chặng soát tool→thiết kế→7 stage fulfillment; lọc theo ngày vào sản xuất `inProductionAt`; snapshot + throughput theo kỳ) | [`documents/FunctionDescription/OrderLifecycle.md`](documents/FunctionDescription/OrderLifecycle.md) | `apps/web/src/pages/home/LifecycleTab.tsx` (+ tab `lifecycle` gate `isAdmin` trong `index.tsx`), `apps/web/src/services/order.ts` → `getLifecycleOverview`, `apps/api/src/modules/order/order.service.ts` → `getLifecycleOverview()` + `toolCheckedAt` set tại updateField/bulkUpdateField/importRework + backfill `onModuleInit`, `apps/api/src/modules/order/order.entity.ts` (`toolCheckedAt`), `apps/api/src/modules/order/order.controller.ts` → `GET /lifecycle-overview` (`@Auth` SuperAdmin/Admin), `packages/shared/dtos/production-order.dto.ts` (`toolCheckedAt` + `LIFECYCLE_STAGE_KEYS` + Lifecycle DTOs incl. `waitingToStart`) |
