...
| **Products** (Config + Xưởng + **Chi tiết sản phẩm/catalog + Biến thể options tự định nghĩa + Vị trí in structured** — Products.md §2.4/§2.5/§2.6: `productCategoryId` (ref `ProductCategory` module riêng, §4)/`collectionIds[]` (ref `Collection`, xem `Collections.md`)/`printMethod`/`printArea`/`sizeChartUrl`/`description`/`itemSpecifics`/đóng gói mặc định + `optionNames[]` (max 3 nhóm option user tự định nghĩa) + `variations[]` (`options[]` align index với optionNames, SKU unique toàn hệ thống, `cost`/`nonShipCost`/`retailPrice`, sinh tổ hợp diff-preserve + BatchEdit) + `printAreas[]` (key ∈ 18 DesignFields — nền cho khách lên đơn CSV). **LƯU Ý naming: `product-config` = module Sản phẩm thực thụ, GIỮ tên legacy không rename — xem Products.md §1**) | [`documents/FunctionDescription/Products.md`](documents/FunctionDescription/Products.md)                         | `apps/web/src/pages/products/` (+ `productForm/{VariationItem,VariantsTable,BatchEditDialog,PrintAreasEditor,variantUtils}`), `apps/api/src/modules/product-config/` (+ migration `onModuleInit` variations→options), `apps/api/src/modules/factory/`, `apps/api/src/modules/machine-type/`, `apps/api/src/modules/product-category/` (§4), shared DTOs `product-config.dto.ts` (`PRODUCT_OPTION_GROUP_MAX`/`PRODUCT_VARIANTS_MAX`/`ProductPrintAreaZod`), `factory.dto.ts`, `machine-type.dto.ts`, `product-category.dto.ts`, workshop_config category `print_method` (`WorkshopConfig.md`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Collections** (bộ sưu tập sản phẩm — trục nhóm cho KHÁCH duyệt/chọn khi lên đơn, KHÁC ProductCategory (phân loại nội bộ + scope Promotion); 1 sản phẩm nhiều collection qua `ProductConfig.collectionIds[]`; catalog khách lọc `?collectionId=`) | [`documents/FunctionDescription/Collections.md`](documents/FunctionDescription/Collections.md) | `apps/web/src/pages/products/CollectionTab.tsx`, `apps/web/src/services/collection.ts`, `apps/api/src/modules/collection/` (mirror product-category), `apps/api/src/modules/customer-portal/customer-catalog.service.ts` (filter `collectionId`), `packages/shared/dtos/collection.dto.ts` |
