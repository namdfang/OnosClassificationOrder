...
| **Fulfillment 5-Stage Workflow** (In→Ép→QC→May→Đóng gói; 1 user per (factory,stage); rework-back về designer hoặc bất kỳ stage trước) | [`documents/FunctionDescription/FulfillmentWorkflow.md`](documents/FunctionDescription/FulfillmentWorkflow.md) | `apps/api/src/modules/fulfillment/` (toàn module: task service + controller + module), `apps/api/src/modules/order/order.entity.ts` (currentFulfillmentStage + fulfillmentStages + fulfillmentTimeline + fulfillmentCompletedAt + makeEmptyStageState), `apps/api/src/modules/user/user.entity.ts` (fulfillmentStage + partial unique index `unique_factory_fulfillment_stage`), `apps/api/src/modules/user/user.service.ts` (Fulfillment validation + isDuplicateStageError E11000 handler), `apps/api/src/modules/designer/designer-task.service.ts` (hook designer.complete → init print stage), `apps/web/src/pages/fulfillment/my-tasks/` (index.tsx 4-tab + ReworkBackDialog), `apps/web/src/pages/users/index.tsx` (stage dropdown khi role=Fulfillment), `apps/web/src/services/fulfillment.ts`, `apps/web/src/components/sidebar/Sidebar.tsx` (entry Task Fulfillment), `apps/web/src/constants/{paths,routerConfig}.ts`, `packages/shared/enums/fulfillment-stage.ts` (5 stage + status + transition action enum), `packages/shared/dtos/production-order.dto.ts` (fulfillment fields trong ProductionOrderZod + FulfillmentTransitionDto + GetFulfillmentMyTasksDto + queue/stats DTOs), `packages/shared/dtos/user.dto.ts` (fulfillmentStage), `packages/shared/constants/permission-catalog.ts` (6 perms mới + Fulfillment preset) |
